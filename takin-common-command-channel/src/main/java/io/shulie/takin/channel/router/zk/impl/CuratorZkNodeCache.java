...
import io.shulie.takin.channel.router.zk.ZkNodeCache;
    ...
    private void doSetNewData(byte[] newData) {
        ...
        if (dataIsCompressed && newData != null) {
            try {
                ...
                logger.warn("ChannelCommand fail to decompress NodeCache of path={}", path, e);
                return;
            }
        } else {
            dataToSet = newData;
                ...
                case RECONNECTED: {
                    if (isConnected.compareAndSet(false, true)) {
                        try {
                            reset();
                            internalRebuild();
                            logger.info("ChannelCommand recovered from RECONNECTED event, path={}", path);
                        } catch (Exception e) {
                            logger.error("ChannelCommand fail to reset after reconnection, path={}", path, e);
                        }
                    }
                    break;
                }
    ...
    private final BackgroundCallback backgroundCallback = new BackgroundCallback() {
        ...
        public void processResult(CuratorFramework client, CuratorEvent event)
            ...
            switch (event.getType()) {
                ...
                default:
                    logger.info("ChannelCommand Unknown CuratorEvent: {}", event.getType());
                    break;
            }
        }
    };
