...
public class AttributeResolver implements Resolver {
    ...
    @Override
    public Map<Object, Object> resolver(Object t) {
        return resolver(t, Thread.currentThread().getContextClassLoader());

    }


    public Map<Object, Object> resolver(Object t, ClassLoader loader) {
        if (String.class.isAssignableFrom(t.getClass())) {
            Method[] methods = Reflect.on(t.toString(), loader).get().getClass().getDeclaredMethods();
            return (Map<Object, Object>) filter.filter(methods);
        } else if (t instanceof Class) {
            Method[] methods = ((Class) t).getDeclaredMethods();
            return (Map<Object,Object>) filter.filter(methods);
        } else {
            Method[] methods = Reflect.on(t).get().getClass().getDeclaredMethods();
            return (Map<Object, Object>) filter.filter(methods);
        }
    }
