...
public final class ConcurrentWeakHashMap<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V> {
    ...
    final HashCodeGenerateFunction hashCodeGenerateFunction;
    final DestroyHook<K, V> hook;
    ...
    private int hashOf(Object key) {
        if (hashCodeGenerateFunction == null) {
            return hash(key.hashCode());
        } else {
            return hash(hashCodeGenerateFunction.hashcode(key));
        }
    ...
    static final class Segment<K, V> extends ReentrantLock {
        ...
        Segment(int initialCapacity, float lf, DestroyHook<K, V> destroyHook) {
            loadFactor = lf;
            hook = destroyHook;
        ...
        void removeStale() {
            ...
            while ((ref = (WeakKeyReference<K>) refQueue.poll()) != null) {
                V v = remove(ref.keyRef(), ref.keyHash(), null, true);
                if (v != null && hook != null) {
                    hook.destroy(ref.get(), v);
                }
            }
        }
    ...
    public ConcurrentWeakHashMap(
        ...
        this(null, null, initialCapacity, loadFactor, concurrencyLevel);
    }
     ...
     *                                  nonpositive.
     */
    public ConcurrentWeakHashMap(HashCodeGenerateFunction hashCodeGenerateFunction, DestroyHook<K, V> hook,
                                 int initialCapacity, float loadFactor, int concurrencyLevel) {
        if (!(loadFactor > 0) || initialCapacity < 0 || concurrencyLevel <= 0) {
            throw new IllegalArgumentException();
        }

        this.hashCodeGenerateFunction = hashCodeGenerateFunction;
        this.hook = hook;
        ...
        for (int i = 0; i < segments.length; ++i) {
            segments[i] = new Segment<K, V>(cap, loadFactor, hook);
        }
    }
    ...
    public ConcurrentWeakHashMap(DestroyHook<K, V> destroyHook, int initialCapacity, float loadFactor) {
        this(null, destroyHook, initialCapacity, loadFactor, DEFAULT_CONCURRENCY_LEVEL);
    }

    public ConcurrentWeakHashMap(HashCodeGenerateFunction hashCodeGenerateFunction, DestroyHook<K, V> destroyHook, int initialCapacity, float loadFactor) {
        this(hashCodeGenerateFunction, destroyHook, initialCapacity, loadFactor, DEFAULT_CONCURRENCY_LEVEL);
    }

    public ConcurrentWeakHashMap(HashCodeGenerateFunction hashCodeGenerateFunction, int initialCapacity, float loadFactor) {
        this(hashCodeGenerateFunction, null, initialCapacity, loadFactor, DEFAULT_CONCURRENCY_LEVEL);
    }
    ...
    public ConcurrentWeakHashMap(DestroyHook<K, V> destroyHook, int initialCapacity) {
        this(null, destroyHook, initialCapacity, DEFAULT_LOAD_FACTOR, DEFAULT_CONCURRENCY_LEVEL);
    }

    public ConcurrentWeakHashMap(HashCodeGenerateFunction hashCodeGenerateFunction, int initialCapacity) {
        this(hashCodeGenerateFunction, null, initialCapacity, DEFAULT_LOAD_FACTOR, DEFAULT_CONCURRENCY_LEVEL);
    }

    public ConcurrentWeakHashMap(HashCodeGenerateFunction hashCodeGenerateFunction, DestroyHook<K, V> destroyHook, int initialCapacity) {
        this(hashCodeGenerateFunction, destroyHook, initialCapacity, DEFAULT_LOAD_FACTOR, DEFAULT_CONCURRENCY_LEVEL);
    }
    ...
    public ConcurrentWeakHashMap(HashCodeGenerateFunction hashCodeGenerateFunction) {
        this(hashCodeGenerateFunction, null, DEFAULT_INITIAL_CAPACITY, DEFAULT_LOAD_FACTOR, DEFAULT_CONCURRENCY_LEVEL);
    }

    public ConcurrentWeakHashMap(DestroyHook<K, V> destroyHook) {
        this(null, destroyHook, DEFAULT_INITIAL_CAPACITY, DEFAULT_LOAD_FACTOR, DEFAULT_CONCURRENCY_LEVEL);
    }
     ...
     * concurrencyLevel (16).
     */
    public ConcurrentWeakHashMap(HashCodeGenerateFunction hashCodeGenerateFunction, DestroyHook<K, V> destroyHook) {
        this(hashCodeGenerateFunction, destroyHook, DEFAULT_INITIAL_CAPACITY, DEFAULT_LOAD_FACTOR, DEFAULT_CONCURRENCY_LEVEL);
    }
    ...
    public interface HashCodeGenerateFunction {
